...
public class RepoIndex implements ResourceIndexer {
	...
	public RepoIndex(LogService log) {
		...
		try {
			...
			Filter nonJarFilter = createFilter("(!" + jarFilterString + ")");
			...
			addAnalyzer(defaultAnalyzer, nonJarFilter);
	...
	private Tag generateResource(File file, Map<String, String> config) throws Exception {
		...
		if (resource == null) {
			log(LogService.LOG_ERROR, MessageFormat.format("Failed to recognize resource {1}", file.getAbsolutePath()), null);
		}
