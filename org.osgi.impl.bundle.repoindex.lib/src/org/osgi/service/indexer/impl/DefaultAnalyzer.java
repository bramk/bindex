...
/**
 * <p>
 * Default Analyzer that is intended to be <b>last</b> in line of any number of
 * analyzers to be consulted. If <b>not</b> set by a previous analyzer, it tries
 * to generate capabilities for <code>Namespaces.NS_IDENTITY</code> and
 * <code>Namespaces.NS_CONTENT</code>.
 * <p>
 * 
 * <p>
 * The identity is extracted from the resource name based on heuristic
 * <code>&lt;identity&gt;[-&lt;version&gt;][.&lt;suffix&gt;]</code>. If no
 * version is found it is omitted. The type always set to <code>file</code>.
 * </p>
 */
class DefaultAnalyzer implements ResourceAnalyzer {
	...
	private void doPlainFileIdentity(Resource resource, List<? super Capability> caps) {
		...
		Builder builder = new Builder().setNamespace(Namespaces.NS_IDENTITY).addAttribute(Namespaces.NS_IDENTITY, name)
				.addAttribute(Namespaces.ATTR_IDENTITY_TYPE, Namespaces.RESOURCE_TYPE_PLAIN_FILE);
		if (version != null)
